...
export function ChartsWorkspace({ samples }: ChartsWorkspaceProps) {
  ...
  const rawData = useMemo<ChartPoint[]>(() => {
    const windowed =
      range === "all"
        ? samples.slice(-1800)
        : samples.slice(-rangeLimits[range]);
  ...
  const summary = [
    { label: "Samples", value: String(chartData.length) },
    {
      label: "Camera FPS",
      value:
        last?.cameraInputFps != null
          ? String(Math.round(last.cameraInputFps * 10) / 10)
          : "--",
    },
    {
      label: "E2E Latency",
      value: last?.e2eDetMs != null ? `${Math.round(last.e2eDetMs)} ms` : "--",
    },
    {
      label: "CPU",
      value: last?.cpuPct != null ? `${Math.round(last.cpuPct)}%` : "--",
    },
  ];
        ...
        <div className="flex flex-wrap gap-2">
          {(["1m", "5m", "15m", "all"] as ChartRange[]).map((option) => (
            <button
              ...
              className={`h-8 rounded-md border px-3 text-xs font-semibold uppercase tracking-[0.12em] transition-all ${
                range === option
      ...
      <PanelShell title="Chart Overview" contentClassName="p-3">
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
          {summary.map((item) => (
            <div
              key={item.label}
              className="rounded-md border border-zinc-700/80 bg-zinc-900/55 px-2.5 py-2"
            >
          ...
          <div className="flex h-[420px] items-center justify-center rounded-md border border-zinc-700/70 bg-zinc-900/55">
            <div className="text-center">
              ...
              <div className="mt-1 text-xs text-zinc-500">
                Need at least 2 samples in the selected range to render charts.
              </div>
            </div>
          </div>
        </PanelShell>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData}>
                  ...
                  <Line
                    yAxisId="left"
                    type="monotone"
                    dataKey="detectionsAvg"
                    stroke="#a1a1aa"
                    dot={false}
                    strokeWidth={1.8}
                    name="Detections Avg (rolling)"
                  />
                  {showReplay && (
                    <Line
                      yAxisId="right"
                      type="monotone"
                      dataKey="replayProgress"
                      stroke="#f59e0b"
                      dot={false}
                      strokeWidth={1.8}
                      name="Replay %"
